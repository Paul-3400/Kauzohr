...
| Uptime | Stabil seit 19. Juli 2026 (alle Services) |
...
| Advanced Settings Passwort | `kauzohr4me` |
...
## 🔗 Symlinks (erstellt am 15. Juli 2026)
...
| Service | Funktion | Status (22.07.) |
|---|---|---|
| `birdnet_recording` | Nimmt Audio auf (alle 15 Sek.) | 🟢 running (3 Tage) |
| `birdnet_analysis` | Analysiert Aufnahmen mit BirdNET-KI | 🟢 running (3 Tage) |
| `birdnet_stats` | Streamlit Dashboard (Port 8501) | 🟢 running (3 Tage) |
| `birdnet_log` | GoTTY Log-Viewer (Port 8080) | 🟢 running (3 Tage) |
| `caddy` | Webserver | 🟢 running |
| `php8.4-fpm` | PHP FastCGI | 🟢 running |
...
```bash
# Status aller BirdNET-Services (NICHT mit Wildcard birdnet* verwenden!)
...
# Journal-Speicherverbrauch prüfen
journalctl --disk-usage
```

**Hinweis:** Die Meldung "journal has been rotated since unit was started, output may be incomplete" ist harmlos – bedeutet nur, dass ältere Log-Einträge aufgeräumt wurden.
...
# Top 10 Arten aller Zeiten
sqlite3 ~/BirdNET-Pi/scripts/birds.db "SELECT Com_Name, COUNT(*) as cnt FROM detections GROUP BY Com_Name ORDER BY cnt DESC LIMIT 10;"
...
# SD-Karten-Gesundheit (Schreibzyklen)
sudo smartctl -a /dev/mmcblk0 2>/dev/null || echo "smartmontools nicht installiert"
```

---

## 🐛 Gelöste Probleme (Sitzung 15. Juli 2026)
...
- Weg von Strassen (Verkehrslärm verwirrt KI)
...
- [ ] Optional: Backup-Strategie für birds.db
